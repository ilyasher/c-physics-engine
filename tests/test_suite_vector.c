...
void test_vec_unit() {
  assert(vec_isclose(vec_unit(VEC_ZERO), VEC_ZERO));
  assert(vec_isclose(vec_unit((Vector){3, 4}), (Vector){3.0/5.0, 4.0/5.0}));
  assert(vec_isclose(vec_unit((Vector){-7, -24}), (Vector){-7.0/25, -24.0/25}));
}
...
void test_vec_len() {
    assert(vec_len(VEC_ZERO) == 0);
    assert(vec_len((Vector){3, 4}) == 5);
    assert(vec_len((Vector){-7, 24}) == 25);
}

void test_vec_angle() {
    assert(vec_angle(VEC_ZERO) == 0);
    assert(vec_angle((Vector){0, 1}) == M_PI / 2);
    assert(vec_angle((Vector){-1, 1}) == M_PI * 3 / 4);
    assert(vec_angle((Vector){1, -1}) == M_PI * 7 / 4);
}
...
int main(int argc, char *argv[]) {
    ...
    DO_TEST(test_vec_unit)
    DO_TEST(test_vec_multiply)
    DO_TEST(test_vec_dot)
    DO_TEST(test_vec_len)
    DO_TEST(test_vec_angle)
